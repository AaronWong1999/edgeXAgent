...
- **测试规范：测试 BWEnews 时，只通过 MCP HTTP 接口测试（`curl http://127.0.0.1:8788/mcp`），绝不直接 import TelegramClient 或访问 session 文件。MCP 就是隔离层，所有外部调用都走 HTTP。**
...
- **TP/SL 是强制的，必须以市价单下单** — 所有 AI 推荐的交易必须自带 TP/SL（市价），通过 edgex-cli `--tp`/`--sl` 参数随主订单一起下单。validate_plan 会拒绝缺少 TP/SL 的计划。典型范围：TP +5-15%，SL -3-8%
